...
1. 每次生成前必读 `data/memory/style-preferences.json`、`data/context/profile.md`、`data/context/preferences.md`、`templates/workflows/`、`templates/platform/`；按任务选择 `data/knowledge/`、`data/feedback/`、`data/analytics/`。`data/operations/` 仅维护/复盘时读取。读取可用分类目录路径，但**写入一律写 `data/` 根主源文件**，随后在 `app/create-video/` 执行 `node scripts/migrate-layout.mjs mirror` 同步镜像（口径见 `PROJECT_INDEX.md` §2.5）；`templates/story/` 是镜像产物，不直接编辑、不新增。
...
7. 完成创作任务后，把任务摘要追加到主源 `data/tasks.md`，发布内容与复盘写入主源 `data/history.md`，确认新的表达偏好后更新主源 `data/preferences.md`；用户反馈直接写入 `data/feedback/`（该目录不在镜像映射内，可自由新增文件）。三类个人数据都有分类目录镜像，写主源后必须在 `app/create-video/` 执行 `node scripts/migrate-layout.mjs mirror` 并保证 `node scripts/migrate-layout.mjs --check` 通过——只改 `data/operations/`、`data/analytics/`、`data/context/` 下的副本会被判定为漂移并报错（映射表见 `PROJECT_INDEX.md` §2.5）。
