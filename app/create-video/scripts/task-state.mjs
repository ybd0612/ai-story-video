...
import { migrateTaskState, backupStateFile } from './task-state-migrations.mjs';
...
const scrubSecrets = (value) => typeof value === 'string' ? secretValues().reduce((result, secret) => result.replaceAll(secret, '[REDACTED]'), value) : value;
const sanitizeEventDetails = (value, key = '') => {
  if (SENSITIVE_FIELD.test(key)) return '[REDACTED]';
  if (Array.isArray(value)) return value.map((item) => sanitizeEventDetails(item));
  if (value && typeof value === 'object') return Object.fromEntries(Object.entries(value).map(([childKey, childValue]) => [childKey, sanitizeEventDetails(childValue, childKey)]));
...
export const readTaskState = async (file) => {
  ...
  try {
    ...
    const error = new Error('Corrupt task state file (invalid JSON)');
  ...
  try {
    const backup = await backupStateFile(file);
    ...
    const last = migrated.migrationHistory && migrated.migrationHistory[migrated.migrationHistory.length - 1];
    if (last) last.backup = backup;
...
export const updateSceneState = async (file, sceneId, kind, patch) => {
  ...
  const existing = state.scenes ? state.scenes[sceneId] ?? {} : {};
...
export const runStage = async (file, state, name, action, options = {}) => {
  ...
  const previous = latest.stages ? latest.stages[name] ?? { status: 'pending', attempts: 0 } : { status: 'pending', attempts: 0 };
  const startedAt = now();
  ...
  try {
    await options.onEvent && options.onEvent('stage_started', { stage: name, attempt: runningStage.attempts });
    await action();
    if (options.validate) await options.validate();
    await options.onEvent && options.onEvent('quality_check_passed', { stage: name });
    ...
    await options.onEvent && options.onEvent('stage_failed', { stage: name, error: message });
